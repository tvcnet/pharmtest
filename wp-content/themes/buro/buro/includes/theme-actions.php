...
if (!function_exists( 'woo_custom_styling')) {
	function woo_custom_styling() {
		...
		// Add CSS to output
		if ($body_color)
			$output .= '#content-wrapper {background:'.$body_color.'}' . "\n";

		if ($body_img)
			$output .= '#content-wrapper {background-image:url( '.$body_img.')}' . "\n";

		if ($body_img && $body_repeat && $body_position)
			$output .= '#content-wrapper {background-repeat:'.$body_repeat.'}' . "\n";

		if ($body_img && $body_position)
			$output .= '#content-wrapper {background-position:'.$body_position.'}' . "\n";
	...
	function woo_custom_typography() {
		...
		if ( $woo_options[ 'woo_typography' ] == "true") {
			...
			if ( $woo_options[ 'woo_font_nav' ] )
				$output .= 'nav, nav .nav a { '.woo_generate_font_css($woo_options[ 'woo_font_nav' ], '1.4').' }' . "\n";

			if ( $woo_options[ 'woo_font_post_title' ] )
				$output .= 'article .title, article .title a, #slides .slide-content h2 a { '.woo_generate_font_css($woo_options[ 'woo_font_post_title' ]).' }' . "\n";
