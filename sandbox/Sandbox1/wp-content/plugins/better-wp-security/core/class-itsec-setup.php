...
class ITSEC_Setup {
	...
	function do_modules() {
		...
		foreach ( $itsec_globals['free_modules'] as $module ) {
			...
			if ( file_exists( $free_modules_folder . '/' . $module . '/setup.php' ) ) {
	...
	private function activate_execute( $upgrade = false ) {
		...
		global $itsec_globals, $itsec_files, $wpdb, $itsec_setup_action, $itsec_old_version;
