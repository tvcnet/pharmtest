...
class ITSEC_Global_Settings {
	...
	public function backup_email() {
		...
		$content = '<textarea id="itsec_global_backup_email" name="itsec_global[backup_email]">' . $emails . '</textarea>';
		$content .= '<p class="description">' . __( 'The email address(es) all database backups will be sent to. One address per line.', 'ithemes-security' ) . '</p>';
	...
	public function email_notifications() {
		...
		$content .= sprintf( '<p class="description">%s<a href="admin.php?page=toplevel_page_itsec_settings">%s</a>%s</p>', __( 'This feature will trigger an email to be sent to the ', 'ithemes-security' ), __( 'notifications email address', 'ithemes-security' ), __( ' whenever a host or user is locked out of the system.', 'ithemes-security' ) );
	...
	public function infinitewp_compatibility() {
		...
		$content = '<input type="checkbox" id="itsec_global_infinitewp_compatibilitys" name="itsec_global[infinitewp_compatibility]" value="1" ' . checked( 1, $infinitewp_compatibility, false ) . '/>';
		$content .= '<label for="itsec_global_email_notifications">' . __( 'Enable InfiniteWP Compatibility', 'ithemes-security' ) . '</label>';
		$content .= sprintf(
	...
	public function lockout_message() {
		...
		$content = '<textarea class="widefat" name="itsec_global[lockout_message]" id="itsec_global_lockout_message" rows="5" >' . $lockout_message . '</textarea>';
		$content .= '<p class="description">' . __( 'The message to display when a computer (host) has been locked out.', 'ithemes-security' ) . '</p>';
	...
	public function log_location() {
		...
		$content .= '<label for="itsec_global_log_location"> ' . __( 'The path on your server where log files should be stored.', 'ithemes-security' ) . '</label>';
	...
	public function nginx_file() {
		...
		$content = '<input class="large-text" name="itsec_global[nginx_file]" id="itsec_backup_nginx_file" value="' . $nginx_file . '" type="text">';
		$content .= '<label for="itsec_backup_nginx_file"> ' . __( 'The path on your server where backup files should be stored.', 'ithemes-security' ) . '</label>';
		$content .= '<p class="description"> ' . __( 'This path must be writable by your website. For added security, it is recommended you do not include it in your website root folder.', 'ithemes-security' ) . '</p>';
	...
	public function notification_email() {
		...
		$content = '<textarea id="itsec_global_notification_email" name="itsec_global[notification_email]">' . $emails . '</textarea>';
		$content .= '<p class="description">' . __( 'The email address(es) all security notifications will be sent to. One address per line.', 'ithemes-security' ) . '</p>';
	...
	public function register_tooltip( $tooltip_modules ) {
		...
		if ( ! isset ( $this->settings['write_files'] ) || $this->settings['write_files'] === false ) {
			...
			$tooltip_modules['writing'] = array(
				...
				'success'   => __( 'Setting Saved', 'ithemes-security' ),
	...
	public function user_lockout_message() {
		...
		$content .= '<p class="description">' . __( 'The message to display to a user when their account has been locked out.', 'ithemes-security' ) . '</p>';
	...
	public function write_files() {
		...
		$content = '<input type="checkbox" id="itsec_global_write_files" name="itsec_global[write_files]" value="1" ' . checked( 1, $write_files, false ) . '/>';
		$content .= '<label for="itsec_global_write_files">' . __( 'Allow iThemes Security to write to wp-config.php', 'ithemes-security' ) . $server_file . '.</label>';
		$content .= sprintf(
			'<p class="description">%s %s %s</p>',
			__( 'Whether or not iThemes Security should be allowed to write to wp-config.php', 'ithemes-security' ),
			$server_file,
