...
class ITSEC_Dashboard_Admin {
	...
	public function admin_script() {
		...
		if ( isset( get_current_screen()->id ) && strpos( get_current_screen()->id, 'itsec' ) !== false ) {
			...
			wp_enqueue_script( 'itsec_dashboard_js', $this->module_path . 'js/admin-dashboard.js', 'jquery' );
		...
		if ( isset ( $statuses['high'][0] ) ) {
			...
			if ( isset ( $statuses['high'] ) ) {
				...
				foreach ( $statuses['high'] as $status ) {
					...
					if ( strpos( $status['link'], 'http:' ) === false ) {
					...
					printf( '<li><p>%s</p><div class="itsec_status_action"><a class="button-primary" href="%s">%s</a></div></li>', $status['text'], $setting_link, __( 'Fix it', 'ithemes-security' ) );
		...
		if ( isset ( $statuses['safe-high'] ) || isset ( $statuses['safe-medium'] ) || isset ( $statuses['safe-low'] ) ) {
			...
			if ( isset ( $statuses['safe-high'] ) ) {
				...
				foreach ( $statuses['safe-high'] as $status ) {
					...
					printf( '<li><p>%s</p><div class="itsec_status_action"><a class="button-secondary" href="%s">%s</a></div></li>', $status['text'], $setting_link, __( 'Edit', 'ithemes-security' ) );
