...
final class ITSEC_Files {
	...
	public function file_writer_init() {
		...
		if ( ( get_site_option( 'itsec_config_changed' ) == '1' || get_site_option( 'itsec_rewrites_changed' ) == '1' ) && isset( $_GET['page'] ) && sanitize_text_field( $_GET['page'] ) == 'toplevel_page_itsec_settings' && isset( $_GET['settings-updated'] ) && sanitize_text_field( $_GET['settings-updated'] ) == 'true' ) {
	...
	public function get_file_lock( $type ) {
		...
		if ( file_exists( $lock_file ) ) {
			...
			$contents = explode( PHP_EOL, @file_get_contents( $lock_file ) );

			if ( isset( $contents[0] ) && @posix_getsid( $contents[0] ) !== false && isset( $contents[1] ) && $itsec_globals['current_time_gmt'] < ( $contents[1] + 180 ) ) {
		...
		@file_put_contents( $lock_file, getmypid() . PHP_EOL . $itsec_globals['current_time_gmt'] );
