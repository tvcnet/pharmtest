...
class ITSEC_Backup_Admin {
	...
	public function dashboard_status( $statuses ) {
		...
		if ( class_exists( 'backupbuddy_api0' ) && sizeof( backupbuddy_api0::getSchedules() ) >= 1 ) {
			...
			$status       = array( 'text' => __( 'Your site is performing scheduled database and file backups.', 'ithemes-security' ), 'link' => '#itsec_backup_enabled', );

		} elseif ( $this->settings['enabled'] === true ) {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your site is performing scheduled database backups but is not backing up files. Consider purchasing or scheduling BackupBuddy to protect your investment.', 'ithemes-security' ), 'link' => 'http://www.ithemes.com/purchase/backupbuddy', );

		} else {

			$status_array = 'high';
			$status       = array( 'text' => __( 'Your site is not performing any scheduled backups.', 'ithemes-security' ), 'link' => '#itsec_backup_enabled', );
	...
	public function backupbuddy_metabox() {
		...
		$content .= '<p class="bub-cta"><a href="http://ithemes.com/purchase/backupbuddy/" target="_blank" class="button-primary" >' . __( 'Learn more about BackupBuddy', 'ithemes-security' ) . '</a></p>';
