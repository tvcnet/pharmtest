...
class ITSEC_File_Change_Admin {
	...
	public function file_list() {
		...
		$content = '<p class="desctiption">' . __( 'Exclude files or folders by clicking the red minus next to the file or folder name.', 'ithemes-security' ) . '</p>';
	...
	public function logs_metabox() {
		...
		if ( isset( $_GET['itsec_file_change_details_id'] ) ) {
			...
			if ( ! class_exists( 'ITSEC_File_Change_Log_Added' ) ) {
				require( dirname( __FILE__ ) . '/class-itsec-file-change-log-added.php' );
			}

			$added_display = new ITSEC_File_Change_Log_Added();

			$added_display->prepare_data_items( $data );
			$added_display->display();

			if ( ! class_exists( 'ITSEC_File_Change_Log_Removed' ) ) {
				require( dirname( __FILE__ ) . '/class-itsec-file-change-log-removed.php' );
			}

			$removed_display = new ITSEC_File_Change_Log_Removed();

			$removed_display->prepare_data_items( $data );
			$removed_display->display();

			if ( ! class_exists( 'ITSEC_File_Change_Log_Changed' ) ) {
				require( dirname( __FILE__ ) . '/class-itsec-file-change-log-changed.php' );
			}

			$changed_display = new ITSEC_File_Change_Log_Changed();
	...
	public function jquery_filetree_ajax() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( $_POST['nonce'] ), 'itsec_jquery_filetree' ) ) {
			die( __( 'Security error!', 'ithemes-security' ) );
		}
	...
	public function sanitize_module_input( $input ) {
		...
		$input['last_run'] = $this->settings['last_run'];
